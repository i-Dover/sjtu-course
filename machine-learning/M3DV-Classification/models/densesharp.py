...
from utils import precision, recall, DiceLoss
...
# def _dense_block(x, n, k):
#     # k = PARAMS['k']
#     x_prev = []
#     x_prev.append(x)
#     for _ in range(n):
#         conv = _conv_block(x, k)
#         x_prev_cat = concatenate(x_prev, axis=-1)
#         x = concatenate([conv, x_prev_cat], axis=-1)
#         x_prev.append(x)
#     return x
def _dense_block(x, n, k):
    # k = PARAMS['k']
...
def get_compiled(loss={"clf": 'categorical_crossentropy',
                       "seg": DiceLoss()},
                 optimizer='adam',
                 metrics={'clf': ['categorical_accuracy', precision, recall],
