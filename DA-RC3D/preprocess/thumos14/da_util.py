...
def dataset_label_parser(meta_dir, split,is_target=False, use_ambiguous=False):
  ...
  if is_target==False:
    ...
    for vid in segment:
      ...
      with open(os.path.join(meta_dir, 'tgt_vidlist.txt'), 'r') as f:
        lines = f.readlines()
        for l in lines:
          vid_name = l.strip().split()[0]
          segment[vid_name] = [[1, 1, 1]]

  if True:
      keys = list(segment.keys())
      keys.sort()
      if is_target == False:
        with open(os.path.join(split + '_segment.txt'), 'w') as f:
          for k in keys:
            f.write("{}\n{}\n\n".format(k, segment[k]))
      else:
        with open(os.path.join(split + '_t_segment.txt'), 'w') as f:
          for k in keys:
            f.write("{}\n{}\n\n".format(k, segment[k]))
