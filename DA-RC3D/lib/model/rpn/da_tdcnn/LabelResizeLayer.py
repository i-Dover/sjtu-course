...
class ImageLabelResizeLayer(nn.Module):
    ...
    def forward(self,x,need_backprop):
        ...
        gt_blob = np.zeros((lbs.shape[0], feats.shape[2], feats.shape[3], feats.shape[4], 1), dtype=np.float32)
        for i in range(lbs.shape[0]):
            lb=np.array([lbs[i]])
            lbs_resize = cv2.resize(lb, (feats.shape[3] ,feats.shape[2]),  interpolation=cv2.INTER_NEAREST)
            for j in range(feats.shape[4]):
                gt_blob[i, 0:lbs_resize.shape[0], 0:lbs_resize.shape[1], j, 0] = lbs_resize
