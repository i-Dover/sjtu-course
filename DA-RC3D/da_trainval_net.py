...
from da_roi_data_layer.roibatchLoader import roibatchLoader
from model.utils.config import cfg, cfg_from_file, cfg_from_list, get_output_dir
from model.utils.net_utils import weights_normal_init, save_net, load_net, \
    adjust_learning_rate, save_checkpoint, clip_gradient

# from model.rpn.c3d import c3d
from model.da_tdcnn.c3d import C3D, c3d_tdcnn
from model.da_tdcnn.i3d import I3D, i3d_tdcnn
from model.da_tdcnn.resnet import resnet_tdcnn
from model.da_tdcnn.eco import eco_tdcnn
...
def train_net(tdcnn_demo, s_dataloader,t_dataloader, optimizer, args):
    ...
    for step in range(len(s_dataloader)):
        data = next(data_iter)
        tgt_data = next(tgt_data_iter)
        s_video_data = data[0].cuda()
        s_gt_twins = data[1].cuda()
        need_backprop = data[3].cuda()
        t_video_data = tgt_data[0].cuda()
        t_gt_twins = tgt_data[1].cuda()
        tgt_need_backprop = tgt_data[3].cuda()
        data_time = time.time() - data_start

        tdcnn_demo.zero_grad()
        rois, cls_prob, twin_pred, rpn_loss_cls, rpn_loss_twin, \
        RCNN_loss_cls, RCNN_loss_twin, rois_label, DA_img_loss_cls,\
        DA_ins_loss_cls, tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,\
        DA_cst_loss,tgt_DA_cst_loss = tdcnn_demo(s_video_data, s_gt_twins, need_backprop, t_video_data, t_gt_twins, tgt_need_backprop)
        loss = rpn_loss_cls.mean() + rpn_loss_twin.mean()+ RCNN_loss_cls.mean() + RCNN_loss_twin.mean()+args.lamda*(\
              ## DA_img_loss_cls.mean()+DA_ins_loss_cls.mean()\
              ## +tgt_DA_img_loss_cls.mean()+tgt_DA_ins_loss_cls.mean()+DA_cst_loss.mean()+tgt_DA_cst_loss.mean())
                DA_ins_loss_cls.mean()+tgt_DA_ins_loss_cls.mean()+DA_cst_loss.mean()+tgt_DA_cst_loss.mean())
        loss_temp += loss.item()
        ...
        if step % args.disp_interval == 0:
           ...
           # loss_DA_img_cls = args.lamda * (DA_img_loss_cls.item() + tgt_DA_img_loss_cls.item()) / 2
            ...
            gt_cnt = data[2].sum().item()

            print("[session %d][epoch %2d][iter %4d/%4d] loss: %.4f, lr: %.2e" \
                  % (args.session, args.epoch, step + 1, len(s_dataloader), loss_temp, args.lr))
          ...
          #  print("\t\t\trpn_cls: %.4f, rpn_twin: %.4f, rcnn_cls: %.4f, rcnn_twin %.4f,\n\t\t\timg_loss %.4f,"  \
           #       "ins_loss %.4f,cst_loss %.4f" % (loss_rpn_cls, loss_rpn_twin, loss_rcnn_cls, loss_rcnn_twin,loss_DA_img_cls,loss_DA_ins_cls,loss_DA_cst))
            print("\t\t\trpn_cls: %.4f, rpn_twin: %.4f, rcnn_cls: %.4f, rcnn_twin %.4f,\n\t\t\tins_loss %.4f,cst_loss %.4f"  \
                 % (loss_rpn_cls, loss_rpn_twin, loss_rcnn_cls, loss_rcnn_twin,loss_DA_ins_cls,loss_DA_cst))
...
if __name__ == '__main__':
    ...
    if args.dataset == "thumos14":
        args.s_imdb_name = "train_data_25fps_flipped_s.pkl"
        args.t_imdb_name = "train_data_25fps_flipped_t.pkl"
        args.s_imdbval_name = "val_data_25fps_s.pkl"
        args.t_imdbval_name = "val_data_25fps_t.pkl"
        args.num_classes = 21
    ...
    # train set
    s_roidb_path = args.roidb_dir + "/" + args.dataset + "/" + args.s_imdb_name
    s_roidb = get_roidb(s_roidb_path)
    t_roidb_path = args.roidb_dir + "/" + args.dataset + "/" + args.t_imdb_name
    t_roidb = get_roidb(t_roidb_path)
    ...
    s_dataset = roibatchLoader(s_roidb,training=True)
    s_dataloader = torch.utils.data.DataLoader(s_dataset, batch_size=args.batch_size,
                                             num_workers=args.num_workers, shuffle=True)
    t_dataset = roibatchLoader(t_roidb,training=False)
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        args.epoch = epoch
        train_net(tdcnn_demo, s_dataloader,t_dataloader, optimizer, args)

        if len(args.gpus) > 1:
            save_name = os.path.join(model_dir, 'tdcnn_{}_{}_{}.pth'.format(args.session, epoch, len(s_dataloader)))
            save_checkpoint({
