...
if __name__ == '__main__':
    ...
    if args.dataset == "thumos14":
        ...
        args.imdbval_name = "test_data_25fps.pkl"
        args.num_classes = 21
...
#        tdcnn_demo = nn.parallel.DataParallel(tdcnn_demo, device_ids = args.gpus)
    ...
    tdcnn_demo = torch.nn.DataParallel(tdcnn_demo)
    cudnn.benchmark = True
    checkpoint = torch.load(load_name)
    tdcnn_demo.load_state_dict({k: v for k, v in checkpoint['model'].items() if k in tdcnn_demo.state_dict()})
